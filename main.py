...
config = json.load(open('config_bg.json', encoding="utf-8"))
...
if sidebar_page_selection == "Въведение":
    survey_introduction(config)
    consent_form()

elif sidebar_page_selection == "Лични данни":
    personal_information()

# Instructions page
elif sidebar_page_selection == "Инструкции":
    instructions()

# Question 1
elif sidebar_page_selection == "Въпрос 1":
    q1_config = config['question1']
    create_question(q1_config)
    if safe_var('professional_category') in ['Публична администрация', 'Изследователски екип']:
        effect_size_question(q1_config, 1)
        
# Question 2
elif sidebar_page_selection == "Въпрос 2":
    q2_config = config['question2']
    create_question(q2_config)
    if safe_var('professional_category') in ['Публична администрация', 'Изследователски екип']:
        effect_size_question(q2_config, 2)
# Question 3
elif sidebar_page_selection == "Въпрос 3":
    q3_config = config['question3']
    create_question(q3_config)
    if safe_var('professional_category') in ['Публична администрация', 'Изследователски екип']:
        effect_size_question(q3_config, 3)
# Question 4
elif sidebar_page_selection == "Въпрос 4":
...
# Question 7
elif sidebar_page_selection == "Въпрос 7":
...
# Question 8
elif sidebar_page_selection == "Въпрос 8":
...
# Question 9
elif sidebar_page_selection == "Въпрос 9":
    q9_config = config['question9']
    create_question(q9_config)
    _, col2, _, _ = st.columns(4)
    with col2:
        st.image("SatSunGraph.png", width=700)
    st.write("По-долу представяме графика, показваща корелацията между температурите в събота и неделя за всеки уикенд през 2022 година във Вашингтон. Както може да се очаква, съществува силна корелация между температурата в събота и температурата в неделя, тъй като някои части от годината са горещи, а други по-хладни. В този случай корелацията е 0,88, което показва много силна положителна връзка.")

# Question 10
elif sidebar_page_selection == "Въпрос 10":
    q10_config = config['question10']
    create_question(q10_config)

# Question 11 - Cost/Benefit
elif sidebar_page_selection == "Въпрос 11":
    cost_benefit_question()

# Question 12 - Risk Aversion
elif sidebar_page_selection == "Въпрос 12":
